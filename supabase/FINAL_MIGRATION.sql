-- =====================================================
-- FINAL MIGRATION - Bulletproof Setup
-- Safe to run multiple times
-- =====================================================

-- =====================================================
-- STEP 1: Drop ALL old indexes and policies
-- =====================================================
...
-- =====================================================
-- STEP 2: Fix existing tables if they have creator_id
-- =====================================================

DO $$
BEGIN
  -- Fix collaboration_requests
...
-- =====================================================
-- STEP 3: Recreate indexes for existing tables
-- =====================================================

-- Collaboration requests (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'collaboration_requests') THEN
...
-- Brand logos (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'brand_logos') THEN
...
-- Creator stats (only if table exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'creator_stats') THEN
...
-- =====================================================
-- STEP 4: Recreate RLS policies for existing tables
-- =====================================================
...
-- =====================================================
-- STEP 7: Add columns to creator_pages
-- =====================================================
...
-- =====================================================
-- STEP 9: Enable RLS and create policies for NEW tables
-- =====================================================
...
SELECT '🎉 MIGRATION COMPLETE! All tables created and configured.' as status;

SELECT
  table_name,
  string_agg(column_name, ', ' ORDER BY ordinal_position) as columns_with_user_id
