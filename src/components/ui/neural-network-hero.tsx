...
import { BackedByDraper } from './backed-by-draper';
...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-black absolute inset-0 w-full h-full" style={{ zIndex: -1 }} aria-hidden>
      <Canvas
        camera={camera}
...
export default function Hero({
  ...
  useEffect(() => {
    ...
    // Initial setup - set elements to be invisible
    const tl = gsap.timeline({
      defaults: { ease: 'power3.out' },
    });

    // Set initial states (removed badge since it has its own animation)
    if (headerRef.current) {
      gsap.set(headerRef.current, { opacity: 0, y: 30, filter: 'blur(10px)' });
    }
    if (paraRef.current) {
    ...
    if (microItems.length > 0) {
      gsap.set(microItems, { opacity: 0, y: 15 });
    }
    ...
    if (paraRef.current) {
      tl.to(paraRef.current, { opacity: 1, y: 0, duration: 0.6 }, 0.5);
    }

    if (ctaRef.current) {
      tl.to(ctaRef.current, { opacity: 1, y: 0, duration: 0.6 }, 0.7);
    }

    if (imageRef.current) {
      tl.to(imageRef.current, { opacity: 1, scale: 1, duration: 0.8 }, 0.6);
    }

    if (microItems.length > 0) {
      tl.to(microItems, { opacity: 1, y: 0, duration: 0.6, stagger: 0.1 }, 0.9);
    }
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 gap-8 px-6 pb-24 pt-36 sm:pt-44 md:px-10 lg:grid-cols-2 lg:gap-12 lg:px-16">
        {/* Left column - Content */}
        <div className="flex flex-col items-start gap-6 sm:gap-8">
          ...
          <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl" style={{ visibility: 'visible' }}>
            {title}
          </h1>

          <p ref={paraRef} className="max-w-xl text-left text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg" style={{ visibility: 'visible' }}>
            {description}
          </p>
        ...
        {/* Right column - Image */}
        <div ref={imageRef} className="hidden lg:flex items-center justify-center lg:justify-end">
          <div className="relative w-full max-w-md lg:max-w-lg">
            <img
              src="/images/hero-creator-profile.png"
              alt="Creator profile with courses and calendar"
